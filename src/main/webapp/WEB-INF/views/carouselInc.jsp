<div class="articles-carousel">
    <div class="article-preview-container">
        <div class="article-title">titlu</div>
        <div class="article-text">Lorem ipsum dolor sit amet, consectetur adipisicing elit. Ipsa laborum libero nam obcaecati officiis, quo sunt unde? Autem beatae, consectetur, illum in iure iusto laborum, magnam nemo praesentium saepe ullam?</div>
        <div class="article-information">
            <div class="article-author">autor</div>
            <a href="" class="article-reference">Read the full article</a>
        </div>
    </div>
    <div class="article-preview-container">
        <div class="article-title">titlu2</div>
        <div class="article-text">Lorem ipsum dolor sit amet, consectetur adipisicing elit. Culpa dolores eligendi enim, eum explicabo, facilis in nostrum numquam obcaecati quas reiciendis repellendus repudiandae rerum saepe suscipit temporibus velit voluptate voluptatibus!Lorem ipsum dolor sit amet, consectetur adipisicing elit. Ipsa laborum libero nam obcaecati officiis, quo sunt unde? Autem beatae, consectetur, illum in iure iusto laborum, magnam nemo praesentium saepe ullam?</div>
        <div class="article-information">
            <div class="article-author">autor</div>
            <a href="" class="article-reference">Read the full article</a>
        </div>
    </div>
</div>


<%--this is the sample; when used, the id will be change and the div will be shown !!! --%>
<div id="sample-article-container" class="article-preview-container" style="display: none;">
    <div class="article-title"></div>
    <div class="article-text"></div>
    <div class="article-information">
        <div class="article-author"></div>
        <a href="" class="article-reference">Read the full article</a>
    </div>
</div>
